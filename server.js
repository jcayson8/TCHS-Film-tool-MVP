...
async function getBreakdown(team, gameName = '') {
  const values = [team];
...
app.get('/api/breakdown', async (req, res, next) => {
  try {
    ...
    res.json(await getBreakdown(team, gameName));
  } catch (error) {
    next(error);
  }
});
...
app.get('/api/coach-notes', async (req, res, next) => {
  try {
    ...
    const gameName = text(req.query.gameName);
    if (!team) return res.status(400).json({ error: 'Team is required' });
    const breakdown = await getBreakdown(team, gameName);
    res.json({
      team,
      gameName: gameName || null,
...
app.get('/api/formation-matchups', async (req, res, next) => {
  try {
    ...
    res.json({ team, gameName: gameName || null, totalSnaps: result.rowCount, matchups });
  } catch (error) {
    next(error);
  }
});
